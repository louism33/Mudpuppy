...
EngineMinimaxV3::EngineMinimaxV3(int maxDepth, bool printInfo, EvalBase *evaluator, string name,
                                 TimePoint timeLimit) {
...
uint32_t EngineMinimaxV3::getBestMoveInt(Board &board) {
    return getIndexLowestBit(getBestMove(board));
}

int EngineMinimaxV3::getDisplayScoreOfMove(Board &board) {
    return aiMoveScore;
}

uint64_t EngineMinimaxV3::getNps(long nodes, long time) {
    if (time == 0) {
        return 0;
    }
...
void EngineMinimaxV3::reset() {
    totalNodes = 0;
    age++;
}

void EngineMinimaxV3::fullReset() {
    reset();
    age = 0;
    tt.resetTT();
}

uint64_t EngineMinimaxV3::getBestMove(Board &board) {
    reset();
...
bool EngineMinimaxV3::stopSearch() {
    if (!running) {
        return true;
    }
...
uint64_t EngineMinimaxV3::iterativeDeepeningSearch(Board &board) {
    uint64_t bestMove = 0;
...
uint64_t
EngineMinimaxV3::principleVariationSearch(Board &board, int depth, int ply, int alpha, int beta, bool extended,
                                          bool passed) {
...
void EngineMinimaxV3::setTimeLimitMillis(TimePoint limit) {
    this->maxDepth = absoluteMaxDepth;
    this->timeLimit = limit;
}

void EngineMinimaxV3::setDepthLimit(int depth) {
    this->maxDepth = depth;
